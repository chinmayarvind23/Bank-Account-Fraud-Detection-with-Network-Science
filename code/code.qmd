...
calculate_jaccard <- function(node, typical_fraud_node, numeric_cols, binary_cols) {
  # Binary comparison for binary fields
  ...
  # Combine matches
  match_count <- sum(binary_match, numeric_match)
  total_count <- length(binary_cols) + length(numeric_cols)
...
for (i in 1:nrow(fraud_transactions)) {
  ...
  # Jaccard similarity
  jaccard_similarity <- calculate_jaccard(node, typical_fraud_node, numeric_columns, binary_columns)
  ...
  # Cosine similarity
  cosine_similarity <- calculate_cosine(node, typical_fraud_node, all_cols)
