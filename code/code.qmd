...
```{r}
# Ensure node_id is of type character in fraud_transactions
fraud_transactions <- fraud_transactions %>%
  mutate(node_id = as.character(node_id))

# Add the typical fraud node as a new row
...
vertices <- bind_rows(fraud_transactions, typical_node_row)

# Normalize numeric fields for Cosine similarity
...
# Normalize numeric data in both fraud_transactions and typical_node_row
fraud_transactions[numeric_columns] <- scale(fraud_transactions[numeric_columns])
typical_node_row[numeric_columns] <- scale(typical_node_row[numeric_columns])

# Define thresholds
numeric_tolerance <- 0.1  # Tolerance for Jaccard similarity in numeric fields
jaccard_threshold <- 0.5
cosine_threshold <- 0.7

# Define Jaccard similarity
calculate_jaccard <- function(node, typical_fraud_node, numeric_cols, binary_cols, numeric_tolerance) {
  # Ensure matching columns for comparison
  binary_match <- node[binary_cols] == typical_fraud_node[binary_cols]
  numeric_match <- abs(node[numeric_cols] - typical_fraud_node[numeric_cols]) <= numeric_tolerance

  # Combine matches
  matches <- sum(binary_match, numeric_match, na.rm = TRUE)
  total_features <- length(binary_cols) + length(numeric_cols)
  
  return(matches / total_features)
}

# Define Cosine similarity
calculate_cosine <- function(node, typical_fraud_node, all_cols) {
  vec1 <- as.numeric(node[all_cols])
  vec2 <- as.numeric(typical_fraud_node[all_cols])
  
  # Dot product and magnitudes
  dot_product <- sum(vec1 * vec2, na.rm = TRUE)
  magnitude1 <- sqrt(sum(vec1^2, na.rm = TRUE))
  magnitude2 <- sqrt(sum(vec2^2, na.rm = TRUE))
...
# Define all columns
binary_columns <- c("keep_alive_session", "foreign_request", "email_is_free", "fraud_bool", "phone_home_valid")
all_cols <- c(binary_columns, numeric_columns)

# Compare each fraud node to the typical node
for (i in 1:nrow(fraud_transactions)) {
  node <- fraud_transactions[i, , drop = FALSE] # Extract row
  
  # Jaccard similarity
  jaccard_similarity <- calculate_jaccard(node, typical_node_row, numeric_columns, binary_columns, numeric_tolerance)
  if (jaccard_similarity >= jaccard_threshold) {
    edges_jaccard <- bind_rows(edges_jaccard, data.frame(
      node_id_1 = "TypicalNode",
  ...
  # Cosine similarity
  cosine_similarity <- calculate_cosine(node, typical_node_row, all_cols)
  if (cosine_similarity >= cosine_threshold) {
    edges_cosine <- bind_rows(edges_cosine, data.frame(
      node_id_1 = "TypicalNode",
      node_id_2 = node$node_id,
...
# Create graphs
graph_jaccard <- graph_from_data_frame(d = edges_jaccard, vertices = vertices, directed = FALSE)
graph_cosine <- graph_from_data_frame(d = edges_cosine, vertices = vertices, directed = FALSE)

# Plot graphs
par(mfrow = c(1, 2))
plot(
  graph_jaccard, 
  main = "Jaccard Similarity Graph",
  vertex.color = ifelse(V(graph_jaccard)$name == "TypicalNode", "red", "lightblue"),
  vertex.size = ifelse(V(graph_jaccard)$name == "TypicalNode", 10, 3),
  edge.width = E(graph_jaccard)$similarity * 2,
  layout = layout_with_fr
)
mtext("Edges represent nodes with Jaccard similarity ≥ 0.5", side = 3, line = 0.5)

plot(
  graph_cosine, 
  main = "Cosine Similarity Graph",
  vertex.color = ifelse(V(graph_cosine)$name == "TypicalNode", "red", "lightgreen"),
  vertex.size = ifelse(V(graph_cosine)$name == "TypicalNode", 10, 3),
  edge.width = E(graph_cosine)$similarity * 2,
  layout = layout_with_fr
)
mtext("Edges represent nodes with Cosine similarity ≥ 0.7", side = 3, line = 0.5)

# Display metrics
print(paste("Number of nodes in the graph:", vcount(graph_jaccard)))
print(paste("Number of edges in Jaccard similarity graph:", gsize(graph_jaccard)))
print(paste("Number of edges in Cosine similarity graph:", gsize(graph_cosine)))
```
